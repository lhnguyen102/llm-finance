...
class FinetuningModel:
    ...
    def _training_step(self, input_ids, labels):
        with self.ctx:
            _ = self.peft_net(input_ids, labels)
            loss = self.peft_net.last_loss
            return loss / self.cfg.gradient_accumulation_steps

    def _logging(self, iter_num, loss, lr):
        if iter_num % self.cfg.log_interval == 0:
            time_end = time.perf_counter()
    ...
    def sample(self) -> None:
        ...
        # Load the model
        peft_net = self.load_adapter_model()
        start = ""
    ...
    def get_sentiment_analysis(self, prompts: Union[List[int], List[str]]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            with self.ctx:
                probs = peft_net.compute_probs(prompt_ids, temperature=1.0, top_k=300)
    ...
    def load_adapter_model(self) -> None:
        ...
        # Pretrained model
        target_modules = [
            n for n, _ in pretrained_net.named_modules() if any(x in n for x in ("query", "value"))
        ]
